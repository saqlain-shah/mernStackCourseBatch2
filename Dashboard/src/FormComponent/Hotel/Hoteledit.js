...
function HotelEdit() {
  ...
  const myFormik = useFormik({
    ...
    validate: (values) => {
      ...
      // Implement validation rules for your hotel fields
      // Example:
      if (!values.name) {
    ...
    onSubmit: async (values) => {
      try {
        ...
        setLoading(false);
        navigate(`/hotel-view/${params.id}`); // Redirect to the hotel view page after updating
      } catch (error) {
        console.log(error);
        setLoading(false);
      }
  ...
  return (
    <div>
      ...
      <form onSubmit={myFormik.handleSubmit}>
        {/* Render input fields for editing hotel information */}
        <div>
  <label>Name</label>
  <input
    name="name"
